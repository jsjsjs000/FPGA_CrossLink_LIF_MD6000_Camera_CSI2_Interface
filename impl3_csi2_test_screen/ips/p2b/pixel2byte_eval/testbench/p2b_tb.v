...
begin
  ...
  if ( exp_byte_count!= actual_byte_count) begin 
    $display("---------------------------------------------");
    $display("*** ERROR: Actual and Expected byte counts are not equal***");
    $display("**** INFO: Actual byte Count is %0d", actual_byte_count);
    $display("**** INFO: Expected byte Count is %0d", exp_byte_count);
    $display("---------------------------------------------");
    $display("********** TEST FAILED **********");
    $display("---------------------------------------------");
...
// DSI_FORMAT == 0
  ...
  initial begin
    forever begin
      ...
      repeat (wc_delay+1) begin
      `else
      repeat (wc_delay) begin
      `endif
         @(posedge byte_clk_i);
      end
      txfr_en_i <= 0;
